    ...
    <article className="rounded-2xl border overflow-hidden bg-white hover:shadow-sm transition-shadow">
      <div className="aspect-video bg-slate-50 grid place-items-center overflow-hidden">
        {isImg ? (
          ...
          <div className="grid place-items-center text-center p-6">
            <div className="h-12 w-12 rounded-lg grid place-items-center bg-[#007A33]/10 text-[#007A33] mx-auto">
      ...
      <div className="p-3 grid gap-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <span className="inline-flex items-center text-xs rounded-full border px-2 py-0.5 text-[#007A33] border-[#007A33]">{item.type}</span>
        </div>
        <p className="text-xs text-slate-600 truncate">{item.issuer || '—'} · {item.semesterTerm.toUpperCase()} {item.year}</p>
        <div className="mt-1 flex items-center justify-between gap-2">
          <a href={item.data} download={item.fileName} target="_blank" rel="noreferrer" className="inline-flex items-center rounded-lg px-3 py-1.5 text-sm bg-[#007A33] text-white hover:bg-[#CC8A00]">Abrir</a>
          <button onClick={()=>onRequestDelete(item)} className="inline-flex items-center rounded-lg px-3 py-1.5 text-sm border hover:border-[#CC8A00]">Eliminar</button>
        </div>
      </div>
    </article>
  )
